...
const (
	...
	P_REG = 2
	C_REG = 3
	PCM   = 4 // Producer Consumer Message

	// responses
	R_ECHO  = 101
	R_P_REG = 102
	R_C_REG = 103
	R_PCM   = 104 // Response Producer Consumer Message
)
...
func WriteMessageToStream(rw *bufio.ReadWriter, msg Message) error {
	...
	if msg.C_REG != nil {
		return writeToStreamWithType(rw, C_REG, string(msg.C_REG.ToByte()))
	}
	if msg.R_C_REG != nil {
		if err := writeToStreamWithType(rw, R_C_REG, string(msg.R_C_REG)); err != nil {
			return err
		}
	}
	if msg.PCM != nil {
...
func ParseMessage(stream_msg []byte) *Message {
	...
	case C_REG:
		c := ConsumerRegisterMessage{}
		c.FromByte(stream_msg[1:])
		return &Message{C_REG: &c}
	case R_C_REG:
		if len(stream_msg) < 2 {
			return nil
		}
		return &Message{R_C_REG: stream_msg[1:]}
	case PCM:
